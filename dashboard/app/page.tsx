      ...
      <section className="nx-workspace" id="nx-top">
        ...
        <section className="nx-section" id="nx-holdings">
          ...
          <div className="nx-holding-grid">
            <article className="nx-glass nx-stock-bay"><header><span>股票持仓 / STOCKS</span><strong>02</strong></header>{stocks.map(stock => <div className="nx-stock-row" key={stock.code}><div><small>{stock.code}</small><b>{stock.name}</b><span>{stock.sector}</span></div><div className="nx-score-dial nx-score-pending"><strong>—</strong></div><Lamp tone="gray" text="待接行情" /><p>{stock.watch}；{stock.direction}</p></div>)}</article>
